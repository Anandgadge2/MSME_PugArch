            ...
            <CardContent className="p-0">
              ...
              <div className="max-h-[560px] divide-y divide-slate-100 overflow-y-auto">
                {pagedDeliveries.map((shipment, index) => (
                  <button key={shipment.id} type="button" onClick={() => setSelectedId(shipment.id)} className={cn('w-full p-4 text-left hover:bg-slate-50', selected?.id === shipment.id && 'bg-slate-50/70')}>
            ...
            <Card className="overflow-hidden">
              <div className="flex flex-col gap-3 bg-[#0f172a] px-5 py-4 text-white md:flex-row md:items-center md:justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div><h3 className="text-xs font-black uppercase tracking-widest">Logistics Tracker</h3><p className="mt-1 text-[10px] font-semibold text-slate-100">{selected.purchaseOrder?.poNumber} | {selected.carrierName || 'Carrier pending'}</p></div>
                </div>
...
function StatusBadge({ status, dark = false }: { status?: string; dark?: boolean }) {
  ...
  return <Badge className={cn('shrink-0 rounded-lg px-3 py-1 text-[9px] font-black uppercase tracking-wide', value === 'DELIVERED' && 'border-green-200 bg-green-50 text-green-700', value === 'DELAYED' && 'border-red-200 bg-red-50 text-red-700', ['DISPATCHED', 'IN_TRANSIT', 'OUT_FOR_DELIVERY'].includes(value) && 'border-blue-200 bg-slate-50 text-[#12335f]', !dark && !['DELIVERED', 'DELAYED', 'DISPATCHED', 'IN_TRANSIT', 'OUT_FOR_DELIVERY'].includes(value) && 'border-slate-200 bg-slate-50 text-slate-600', dark && 'border-white/20 bg-white/10 text-white')}>{value.replace(/_/g, ' ')}</Badge>;
}
