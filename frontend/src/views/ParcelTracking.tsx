  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col gap-3 border-b border-slate-200 pb-4 md:flex-row md:items-end md:justify-between">
        <div>
          <p className="text-[10px] font-black uppercase tracking-widest text-[#1d4ed8]">Procurement Logistics</p>
        ...
        <CardContent className="space-y-3 p-4">
          ...
          <div className="grid grid-cols-1 gap-3 md:grid-cols-[1fr_220px_auto]">
            <div className="relative">
              ...
              <input value={searchTerm} onChange={event => setSearchTerm(event.target.value)} placeholder="Search tracking no., PO, vendor, location..." className="h-10 w-full rounded-lg border border-slate-200 bg-white pl-10 pr-3 text-xs font-semibold outline-none focus:ring-2 focus:ring-[#1d4ed8]/20" />
            </div>
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex items-start justify-between gap-3">
                          <div>
                            <p className="break-words text-xs font-black text-[#1d4ed8]">{shipment.trackingNumber || `DLV-${shipment.id}`}</p>
                            <p className="mt-1 break-words text-sm font-black text-blue-900">{shipment.purchaseOrder?.title || shipment.purchaseOrder?.poNumber}</p>
                          </div>
            ...
            <Card className="overflow-hidden">
              ...
              <div className="space-y-5 bg-white p-5">
                ...
                <div className="rounded-xl border border-slate-100 bg-slate-50 p-4">
                  <div className="flex items-start gap-3">
                    <MapPin className="mt-0.5 h-4 w-4 text-[#1d4ed8]" />
                ...
                <div className="relative space-y-6 py-2">
                  ...
                  {(selected.events?.length ? selected.events : [{ id: 0, status: selected.status, location: selected.currentLocation, createdAt: selected.expectedDelivery }]).map(event => (
                    <div key={event.id} className="relative flex items-center gap-4">
                      <div className="z-10 flex h-7 w-7 items-center justify-center rounded-full bg-[#1d4ed8] text-white shadow-sm">
                      ...
                      <div className="min-w-0 flex-1"><p className="break-words text-[11px] font-black uppercase text-blue-900">{event.status || 'Tracking created'}</p><p className="text-[10px] font-semibold text-slate-400">{event.location || event.remarks || formatDate(event.createdAt)}</p></div>
                    </div>
                  ))}
                </div>
              </div>
...
function MetricCard({ label, value, hint, icon: Icon }: { label: string; value: number; hint: string; icon: any }) {
  return <Card><CardContent className="flex items-center justify-between p-4"><div><p className="text-[9px] font-black uppercase tracking-widest text-slate-400">{label}</p><p className="mt-1 text-2xl font-black text-slate-950">{value}</p><p className="mt-1 text-[10px] font-bold uppercase tracking-wide text-slate-500">{hint}</p></div><div className="flex h-10 w-10 items-center justify-center rounded-lg bg-[#1d4ed8] text-white"><Icon className="h-5 w-5" /></div></CardContent></Card>;
}

function InfoBox({ label, value }: { label: string; value: string }) {
  return <div className="rounded-lg border border-slate-100 bg-slate-50 px-3 py-2"><p className="text-[9px] font-black uppercase tracking-widest text-slate-400">{label}</p><p className="mt-1 break-words text-xs font-black text-blue-900">{value}</p></div>;
}
