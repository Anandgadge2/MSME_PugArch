  ...
  return (
    <div className="flex min-h-dvh items-center justify-center bg-slate-50 px-4 py-8">
      <div className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-6 shadow-xl">
        <h1 className="text-xl font-black text-[#1d4ed8]">Reset Password</h1>
        ...
        <form onSubmit={sent ? resetPassword : requestReset} className="mt-6 space-y-4">
          <div>
            ...
            <input
              ...
              disabled={sent}
              required
              className="mt-1 h-11 w-full rounded-xl border border-slate-200 px-3 text-sm font-semibold outline-none focus:ring-2 focus:ring-[#1d4ed8]/20"
            />
          </div>
            ...
            <>
              <div>
                ...
                <input
                  ...
                  required
                  className="mt-1 h-11 w-full rounded-xl border border-slate-200 px-3 text-center text-lg font-black tracking-[0.5em] outline-none focus:ring-2 focus:ring-[#1d4ed8]/20"
                />
              </div>
              <div>
          ...
          <Button type="submit" disabled={isLoading} className="h-11 w-full rounded-xl bg-[#1d4ed8] font-black uppercase tracking-widest text-white">
            {isLoading ? 'Please wait...' : sent ? 'Reset Password' : 'Send Reset Code'}
          </Button>
        </form>

        <Link href="/login" className="mt-4 block text-center text-xs font-black uppercase tracking-widest text-[#1d4ed8] underline underline-offset-4">
          Back to login
        </Link>
      </div>
    </div>
