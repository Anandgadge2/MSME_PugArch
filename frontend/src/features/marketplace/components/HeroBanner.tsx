...
export function HeroBanner({ banners }: Props) {
    ...
    return (
        ...
        >
            ...
            {/* Top Bar CTA Overlay (Anchored to top of hero section) */}
            <div className="absolute top-3 sm:top-4 lg:top-6 inset-x-0 z-30 pointer-events-none">
                <div className="mx-auto w-full max-w-[1680px] px-4 sm:px-10 md:px-14 lg:px-16 2xl:px-20 flex justify-end">
                    <div className="flex flex-row items-center gap-2 sm:gap-2.5 pointer-events-auto">
                        <button 
                            type="button"
                            onClick={handlePostRequirement} 
                            className="group inline-flex items-center justify-center gap-1.5 h-8 sm:h-9 lg:h-10 px-3.5 sm:px-4 lg:px-5 rounded-full border border-white/40 bg-black/50 backdrop-blur-md text-white text-[11px] sm:text-xs lg:text-sm font-bold hover:bg-white/20 hover:border-[#c8a45c]/70 active:scale-95 transition-all shadow-lg"
                        >
