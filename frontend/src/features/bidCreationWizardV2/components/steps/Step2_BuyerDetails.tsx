...
export default function Step2_BuyerDetails({ data, errors, updateField }: StepComponentProps) {
  const masterData = useMasterData(data.district);
  const fields = [
  ...
  return (
    <div className="grid gap-4 md:grid-cols-2">
      ...
      <FormField label="State" required error={errors.state}><SearchableSelect value={data.state} options={['Maharashtra', 'Other']} onChange={value => updateField('state', value)} allowOther /></FormField>
      <FormField label="District" required error={errors.district}><SearchableSelect value={data.district} options={masterData.districts} onChange={value => updateField('district', value)} allowOther allowNA={false} /></FormField>
