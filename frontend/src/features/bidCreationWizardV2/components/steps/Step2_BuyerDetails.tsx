...
export default function Step2_BuyerDetails({ data, errors, updateField }: StepComponentProps) {
  const masterData = useMasterData(data.district, data.state);
  const fields = [
  ...
  return (
    <div className="grid gap-4 md:grid-cols-2">
      ...
      <FormField label="District" required error={errors.district}>
        <SearchableSelect
          value={data.district}
          options={data.state === 'Maharashtra' ? masterData.districts : []}
          onChange={value => {
      ...
      <FormField label="Taluka" required error={errors.taluka}><SearchableSelect value={data.taluka} options={data.state === 'Maharashtra' ? masterData.talukas : []} onChange={value => updateField('taluka', value)} allowOther /></FormField>
      <FormField label="Village / City / Ward"><SearchableSelect value={data.villageOrCity} options={data.state === 'Maharashtra' ? masterData.villages : []} onChange={value => updateField('villageOrCity', value)} allowOther allowNA /></FormField>
