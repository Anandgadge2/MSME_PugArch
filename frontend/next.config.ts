...
const getBackendUrl = (): string => {
  ...
  if (process.env.VERCEL_URL) {
    ...
    if (vercelUrl.includes("msme-pugarch-frontend")) {
      backendHost = vercelUrl.replace(
        "msme-pugarch-frontend",
        "msme-pugarch-backend",
      );
...
const nextConfig: NextConfig = {
  ...
  async rewrites() {
    ...
    if (process.env.VERCEL_URL && backendUrl) {
      console.log(
        `[next.config] Rewrites: /proxy/:path* → ${backendUrl}/:path*`,
      );
    ...
    return {
      ...
      fallback: [
        ...
        {
          source: "/uploads/:path*",
          destination: `${devBackend}/uploads/:path*`,
        },
      ],
    };
  },
