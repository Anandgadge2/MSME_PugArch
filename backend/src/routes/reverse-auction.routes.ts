...
const resolveAuctionId = async (rawId: number | string): Promise<number | null> => {
  ...
  // 2. If it has a prefix like "RA-", "REQ-", etc. followed by an integer, try direct ID fallback
  const stripped = str.replace(/^(RA-|REQ-|AUC-|PBID-|TENDER-)/i, '');
  ...
  if (Number.isFinite(numStripped) && numStripped > 0) {
    ...
    const linkedByStripped = await db.auction.findFirst({
      where: {
        OR: [
          { linkedRequirementId: numStripped },
          { id: numStripped },
          { linkedBidId: numStripped },
          { tenderId: numStripped }
        ]
      },
      select: { id: true },
      orderBy: { createdAt: 'desc' }
    });
    if (linkedByStripped) return linkedByStripped.id;
  }
  ...
  // Also check if str matches requirement.requirementNumber
  const req = await db.requirement.findFirst({
    where: { OR: [{ requirementNumber: str }, { requirementNumber: `REQ-${str}` }] },
    select: { id: true }
  });
  if (req) {
...
const linkedRequirementSummary = async (auction: any) => {
  ...
  return {
    ...
    paymentTerms: terms.paymentTerms || payload.terms?.paymentTerms || basics.paymentTerms || null,
