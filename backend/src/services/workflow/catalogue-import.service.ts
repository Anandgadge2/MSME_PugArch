...
const detectDownloadedFileType = (
  buffer: Buffer,
  declaredMime: string,
): DownloadedFileType | null => {
  const normalizedMime = declaredMime.split(";")[0].trim().toLowerCase();
  if (buffer.subarray(0, 4).equals(Buffer.from([0x25, 0x50, 0x44, 0x46])))
    return fileTypeForMime("application/pdf") || null;
  if (buffer.subarray(0, 3).equals(Buffer.from([0xff, 0xd8, 0xff])))
    return fileTypeForMime("image/jpeg") || null;
  if (
    buffer
      .subarray(0, 8)
      .equals(Buffer.from([0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x0a]))
  )
    return fileTypeForMime("image/png") || null;
  if (
    buffer.subarray(0, 4).equals(Buffer.from([0x52, 0x49, 0x46, 0x46])) &&
    buffer.subarray(8, 12).equals(Buffer.from([0x57, 0x45, 0x42, 0x50]))
  )
    return fileTypeForMime("image/webp") || null;
  if (
    buffer.subarray(4, 8).equals(Buffer.from([0x66, 0x74, 0x79, 0x70])) &&
    (buffer.subarray(8, 12).toString("latin1").startsWith("avif") ||
      buffer.subarray(8, 12).toString("latin1").startsWith("avis"))
  ) {
  ...
  if (
    ...
    return fileTypeForMime("text/csv") || null;
  return fileTypeForMime(normalizedMime) || null;
};

const fileNameForDownloadedUrl = (
  url: string,
  detectedType: DownloadedFileType,
) => {
  ...
  try {
    ...
    originalName =
      extensionType?.mimeType === detectedType.mimeType
        ? basename
      ...
      try {
        ...
        const extractFromObj = (obj: any): string | null => {
          ...
          if (typeof obj === "object") {
            ...
            if (obj.url && /\.(jpe?g|png|webp)/i.test(obj.url))
              return extractFromObj(obj.url);
...
async function downloadAndUploadUrl(
  ...
  try {
    ...
    if (!res.ok) {
      console.warn(
        `[Catalogue Import] Failed to fetch URL: ${url}, status: ${res.status}`,
      );
      return {
        success: false,
        url,
    ...
    let detectedType = detectDownloadedFileType(
      buffer,
    ...
    if (!detectedType && expectedKind === "image" && depth === 0) {
      ...
      if (
        ...
        if (extractedImageUrl && extractedImageUrl !== url) {
          console.info(
            `[Catalogue Import] Automatically extracted product image "${extractedImageUrl}" from webpage: ${url}`,
          );
          return downloadAndUploadUrl(
            extractedImageUrl,
            userId,
            role,
            entityType,
    ...
    if (expectedKind === "image") {
      try {
        ...
        console.warn(
          `[Catalogue Import] Sharp could not decode image from ${url}:`,
          err?.message,
        );
      }
    }
    ...
    if (!detectedType) {
      console.warn(
        `[Catalogue Import] Skipped URL with unsupported or invalid file content: ${url}`,
      );
      return {
        success: false,
        url,
        reason: "Unsupported or invalid file content",
      };
    }
    if (detectedType.resourceKind !== expectedKind) {
      console.warn(
        `[Catalogue Import] Skipped ${expectedKind} URL with ${detectedType.resourceKind} content: ${url}`,
      );
      return {
        success: false,
        url,
        reason: `File is ${detectedType.resourceKind}, expected ${expectedKind}`,
      };
    }
    ...
    console.warn(
      `[Catalogue Import] Skipped URL ${url}: ${err?.message || "download/upload failed"}`,
    );
    return {
      success: false,
      url,
      reason: err?.message || "Download/upload failed",
    };
  }
}
...
const sanitizeText = (value: unknown, max = 4000) =>
  clean(value).slice(0, max) || null;
...
const STANDARD_ITEM_CONDITIONS = [
  "NEW",
  "REFURBISHED",
  "USED",
  "CUSTOM_MANUFACTURED",
...
const readSheetRows = (workbook: XLSX.WorkBook, sheetName: string) => {
  ...
  const rows = matrix
    ...
    .filter((row) =>
      Object.keys(row).some((k) => k !== "__rowNumber" && clean(row[k])),
    );
  return { headers, rows };
};
...
const assertApprovedSeller = async (actor: WorkflowActor) => {
  ...
  if (
    ...
    throw new ApiError(
      403,
      "Your seller account must be approved before importing catalogue items.",
      "SELLER_NOT_APPROVED",
    );
  }
};
...
export const catalogueImportService = {
  async generateProductTemplate() {
    ...
    const productHeaders = [
      ...
      "Discount Percent",
      "Offer Label",
      "Offer Start Date (YYYY-MM-DD)",
      "Offer End Date (YYYY-MM-DD)",
      "Bulk Deal Available (Yes/No)",
    ...
    const sampleCategory1 =
      categories[0]?.name || "Safety Equipment & Industrial Safety";
    const sampleCategory2 =
      categories.find((c) => /Fastener|Hardware|Mechanical|Metal/i.test(c.name))
        ?.name ||
      categories[1]?.name ||
    ...
    const maxDropdownLength = Math.max(
      categories.length,
      STANDARD_UNITS.length,
      STANDARD_ITEM_CONDITIONS.length,
      3,
    );
    const dropdownRows = [
    ...
    XLSX.utils.book_append_sheet(
      wb,
      XLSX.utils.aoa_to_sheet([productHeaders, ...sampleProducts]),
      "Products",
    );
    XLSX.utils.book_append_sheet(
      wb,
      XLSX.utils.aoa_to_sheet([specHeaders, ...sampleSpecs]),
      "Product Specifications",
    );
    XLSX.utils.book_append_sheet(
      wb,
      XLSX.utils.aoa_to_sheet(productInstructions()),
      "Instructions",
    );
    XLSX.utils.book_append_sheet(
      wb,
      XLSX.utils.aoa_to_sheet(dropdownRows),
      "Dropdown Values",
    );
  ...
  async generateServiceTemplate() {
    ...
    const sampleServiceCategory =
      categories[0]?.name || "Industrial Maintenance Services";
    ...
    const sampleServiceSpecs = [
      [
        ...
        "Licensed Grade-A Electrical Supervisors",
        "",
      ],
      [
        "Annual Industrial Electrical Maintenance & Inspection",
    ...
    XLSX.utils.book_append_sheet(
      wb,
      XLSX.utils.aoa_to_sheet([serviceHeaders, ...sampleServices]),
      "Services",
    );
    XLSX.utils.book_append_sheet(
      wb,
      XLSX.utils.aoa_to_sheet([serviceSpecHeaders, ...sampleServiceSpecs]),
      "Service Specifications",
    );
    XLSX.utils.book_append_sheet(
      wb,
      XLSX.utils.aoa_to_sheet(serviceInstructions()),
      "Instructions",
    );
    XLSX.utils.book_append_sheet(
      wb,
  ...
  async previewImport(
    ...
    if (!file?.buffer)
      throw new ApiError(400, "Excel file is required", "FILE_REQUIRED");
    if (file.size > MAX_FILE_BYTES)
      throw new ApiError(400, "File exceeds 10MB limit", "FILE_TOO_LARGE");
    ...
    if (rows.length === 0) {
      ...
      if (fallback.rows.length === 0)
        throw new ApiError(400, "Workbook contains no data rows", "EMPTY_FILE");
    }
    const dataRows =
      rows.length > 0
        ? rows
        : readSheetRows(workbook, workbook.SheetNames[0] || "").rows;
    if (dataRows.length > MAX_ROWS)
      throw new ApiError(
        400,
        `Maximum ${MAX_ROWS} rows allowed per import`,
        "TOO_MANY_ROWS",
      );
        ...
        ? new Set(
            (
              await db.product.findMany({
                where: { sellerId: actor.id, sku: { not: null } },
                select: { sku: true },
              })
            ).map((p) => clean(p.sku).toLowerCase()),
          )
        : new Set<string>();
    ...
    for (const row of dataRows) {
      ...
      if (gst !== null && (gst < 0 || gst > 40))
        errors.push({
          rowNumber,
          field: "gstRate",
          message: "GST Rate must be between 0 and 40",
          rawData: row,
        });
      if (currency !== "INR")
        errors.push({
      ...
      if (dedupeKey) {
        ...
        if (type === "PRODUCT" && sku && existingSkus.has(sku.toLowerCase())) {
          errors.push({
            rowNumber,
            field: "sku",
            message: `SKU "${sku}" already exists in your catalogue`,
            rawData: row,
          });
        }
      }
      ...
      const specs = specData
        .filter((s) => {
          ...
          const specName = clean(
            col(s, "Product Name", "Name", "Service Name"),
          );
          if (type === "PRODUCT") {
            if (sku && specSku && sku.toLowerCase() === specSku.toLowerCase())
              return true;
            if (
              name &&
              specName &&
              name.toLowerCase() === specName.toLowerCase()
            )
              return true;
            if (name && specSku && name.toLowerCase() === specSku.toLowerCase())
              return true;
            if (sku && specName && sku.toLowerCase() === specName.toLowerCase())
              return true;
            return false;
          } else {
            return Boolean(
              name && specName && name.toLowerCase() === specName.toLowerCase(),
            );
          }
        })
        .map((s) => ({
      ...
      const imageResults = await Promise.all(
        imageUrls.map((url) =>
          downloadAndUploadUrl(
            ...
            actor.id,
            actor.role,
            type === "PRODUCT" ? "catalogue_product" : "catalogue_service",
            "image",
          ),
        ),
      );
      for (const res of imageResults) {
        ...
        if ("assetId" in res) {
          ...
          warnings.push(
            `Row ${rowNumber}: Image "${res.url}" could not be downloaded (${res.reason})`,
          );
        }
      }
      ...
      for (const res of docResults) {
        ...
        if ("assetId" in res) {
          ...
          warnings.push(
            `Row ${rowNumber}: Document "${res.url}" could not be downloaded (${res.reason})`,
          );
        }
      }
      ...
      const basePriceVal =
        type === "PRODUCT" ? price : parseNumber(col(row, "Base Price"));
      const effectiveOriginalPrice =
        rawOriginalPrice ??
        (rawDiscountPrice !== null || rawDiscountPercent !== null
          ? basePriceVal
          : null);
      ...
      if (effectiveOriginalPrice && effectiveOriginalPrice > 0) {
        if (
          ...
          effectiveOriginalPrice > finalDiscountPrice
        ) {
          ...
          finalDiscountPrice =
            Math.round(
              effectiveOriginalPrice * (1 - finalDiscountPercent / 100) * 100,
            ) / 100;
        }
      }
  ...
  async confirmImport(actor: WorkflowActor, batchId: number, publish = false) {
    await assertApprovedSeller(actor);
    const batch = await db.catalogueImportBatch.findFirst({
      where: { id: batchId, sellerId: actor.id },
    });
    if (!batch)
      throw new ApiError(404, "Import batch not found", "BATCH_NOT_FOUND");
    if (batch.status !== "PREVIEWED" && batch.status !== "FAILED") {
      throw new ApiError(
        400,
        "Import batch already processed",
        "BATCH_ALREADY_PROCESSED",
      );
    }

    const rows = Array.isArray(batch.previewData)
      ? (batch.previewData as Record<string, unknown>[])
      : [];
    if (rows.length === 0) {
      await db.catalogueImportBatch.update({
        where: { id: batchId },
        data: { status: "FAILED" },
      });
      ...
      await db.$transaction(
        async (tx) => {
          for (const row of rows) {
            const status =
              publish && row.status === "ACTIVE" ? "ACTIVE" : "DRAFT";
            const {
              specifications,
              rowNumber,
              categoryId,
    ...
    await auditWorkflow(
      actor,
      "workflow.catalogue.import_confirmed",
      "catalogue_import_batch",
      batchId,
      { type: batch.type, successCount },
    );
  ...
  async getErrors(actor: WorkflowActor, batchId: number) {
    ...
    if (!batch)
      throw new ApiError(404, "Import batch not found", "BATCH_NOT_FOUND");
    return db.catalogueImportError.findMany({
      where: { batchId },
  ...
  async exportErrorReport(actor: WorkflowActor, batchId: number) {
    ...
    if (!batch)
      throw new ApiError(404, "Import batch not found", "BATCH_NOT_FOUND");
    const errors = await db.catalogueImportError.findMany({
      where: { batchId },
      orderBy: { rowNumber: "asc" },
    });
    const wb = XLSX.utils.book_new();
