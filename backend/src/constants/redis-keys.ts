...
export const redisKeys = {
  ...
  cacheMarketplaceFeaturedCategories: () => 'cache:marketplace:categories:featured:v1',
  cacheMarketplaceHome: () => 'cache:marketplace:home:v2',
  cacheMarketplaceHomeLayout: (hash: string) => `cache:marketplace:home-layout:v2:${routeKey(hash)}`,
  ...
  cacheAdminKpiSummary: (timeframe?: string, role?: string) => `cache:admin:kpi-summary:${timeframe || 'all'}:${role || 'all'}`,
